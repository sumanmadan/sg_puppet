...
String lastLoadDateSql = "select max(load_date) as load_date from puppet_mast where lower(nodename) like 'tct%' ";
infoSql = "select * from puppet_mast where factname in ( 'ipaddress' , 'serialnumber',  'productname', 'specs_fa_version', 'framework_version', 'util_version', 'prober_ip_nic1' ,'prober_ip_nic2','hp4070_version' ) "+ 
	      " and  lower(nodename) like 'wts%'   ";
...
try{
	...
	toPrintTable = "<p>" + " Last Updated for WTS : " + tdate + "</p>";
...
toPrintTable += "<table id = \"wts\"  class=\"display compact\" cellspacing=\"0\" width=\"100%\" >" + "<thead><tr>";
...
toPrintAHref += "<a href=\"javascript:void(0);\" class=\"LinkButton\"" + "onclick=\"fnShowHide(0);\"" + ">Toggle Server<br></a>";
	...
	System.out.println("3");
		...
		for (Map.Entry<String, String> en : serverMap.entrySet()) {
			...
			if ( tmpServer.startsWith("wts") || tmpServer.startsWith("WTS") ) {
				//toPrintTable += "<td>" + 4080 + "</td>";
				toPrintTable += "<td style='background-color:"  +  "#F9EEFF" +  "'>" + 4080 + "</td>";  

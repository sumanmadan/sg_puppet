...
String lastLoadDateSql = "select max(load_date) as load_date from puppet_mast where lower(nodename) like 'tct21%' ";
infoSql =  "select * from puppet_mast where factname in ( 'ipaddress' , 'serialnumber', 'fqdn' , 'productname', 'specs_fa_version', 'framework_version', 'util_version', 'prober_ip_nic1' ,'hp4070_version'," +
		   " 'prober_driver' , 'vgem' , 'csv_formatter', 'smart_test_version' ) "  +
	      " and  lower(nodename) like 'tct21%'   ";
...
try{
	...
	toPrintTable = "<p>" + " Last Updated for TCT : " + tdate + "</p>";
...
toPrintTable += "<table id = \"tct\"  class=\"display compact\" cellspacing=\"0\" width=\"100%\" >" + "<thead><tr>";
...
toPrintAHref += "<a href=\"javascript:void(0);\" class=\"LinkButton\"" + "onclick=\"fnShowHide(0);\"" + ">Toggle Server<br></a>";
	try{
		...
		while( rs.next() ){
		      ...
		      if ( fact.equalsIgnoreCase("ipaddress") || fact.equalsIgnoreCase("fqdn") || fact.equalsIgnoreCase("prober_ip_nic1") || fact.equalsIgnoreCase("serialnumber")) {
		    		  toColorFlag = "No";
		      } else {
		    	  toColorFlag = "Yes";
		      }
		  	...
		  	if ( colorMap.containsKey(colorKey)) {
		   		...
		   		//Color a = new Color((int)(Math.random() * 0x1000000));
			  		...
			  		if ( toColorFlag.equalsIgnoreCase("Yes")) {
		  				...
		  				int green =random.nextInt(256) +  random.nextInt(256)/2;
		  				int blue = random.nextInt(256) + random.nextInt(256)/2;
	...
	System.out.println("3");
		...
		for (Map.Entry<String, String> en : serverMap.entrySet()) {
			...
			if ( tmpServer.startsWith("tct") || tmpServer.startsWith("TCT") ) {
				//toPrintTable += "<td>" + 93000 + "</td>";
				toPrintTable += "<td style='background-color:"  +  "#DBF0F7" +  "'>" + 93000 + "</td>";  
			}
			...
			for (Map.Entry<String, String> entry : factsMap.entrySet()) {
				...
				if ( sMap.containsKey(getVal)) {
				   ...
				   //System.out.println("Color " + colorMap.get(getVal));
				...
				if ( colorMap.containsKey(colorKey) ) {
					...
					bgcolor = colorMap.get(colorKey);
					System.out.println(colorKey + "  Bug ..." + colorMap.get(colorKey));
					
				}
		   		 toPrintTable += "<td style='background-color:"  +  colorMap.get(colorKey) +  "'>" + tdVal + "</td>";  
